...
a curated draft answer verbatim, or None so the caller can refuse. The YAML
...
REFUSAL = (
    "I'm sorry, I can only share a few curated general topics. For anything else, "
...
def lookup(question: str) -> Optional[Dict[str, Any]]:
    """Return an unambiguous best-matching curated entry, or None.

    Match = the question text contains one of an entry's `keywords`. The entry
    with the most keyword hits wins; ties return None. Deterministic.
    """
    q = (question or "").lower()
    if not q.strip():
        return None
    ...
    for entry in entries:
        ...
        hits = sum(1 for k in kws if k and re.search(r"(?<!\w)" + re.escape(k) + r"(?!\w)", q))
        if hits > best_hits:
            best_hits, best = hits, entry
            tied = False
        elif hits and hits == best_hits:
            tied = True
    return best if best_hits > 0 and not tied else None
